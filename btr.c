...
void main()
{
	...
	struct node*delete9struct node*,int);
	int opt,data;
do
{
	printf("1. insert \n");
...
struct node*insert(struct node*bst,int data)
{
	...
	t->data=data;
	t->left=1->right=(struct node*)0;
	
	if(bst==0)
		bst=t;
		else
		(
		while(t1->data!=data&&t1-left=0)
		if(data < t->data)
			t1=t1->left;
		else
			t1=t1->right;
...
struct node*search(struct node*est,int data)
{
